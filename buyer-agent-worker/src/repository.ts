...
export async function fetchSellerCandidates(
  ...
  const sellersResult = await sql.unsafe<SellerRow[]>(
    `
      SELECT
        id,
        marketplace_id,
  ...
  const agentIds = sellersResult.map((row) => row.id)
  const plansResult = await sql.unsafe<SellerPlanRow[]>(
    `
      SELECT
        services.agent_id,
...
export async function createRun(sql: Sql, config: BuyerAgentConfig): Promise<BuyerAgentRunRow> {
  const result = await sql.unsafe<BuyerAgentRunRow[]>(
    `
...
export async function failRun(sql: Sql, runId: string, errorMessage: string): Promise<void> {
  await sql.unsafe(
    `
      UPDATE buyer_agent_runs
      SET
...
export async function insertJudgment(sql: Sql, input: JudgmentInsertInput): Promise<void> {
...
export async function insertSetupFailure(sql: Sql, input: SetupFailureInput): Promise<void> {
  await sql.begin(async (tx) => {
    await tx.unsafe(
      `
        INSERT INTO buyer_agent_judgments (
...
export async function markSellerVerified(sql: Sql, agentId: string): Promise<boolean> {
